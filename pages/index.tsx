...
const Home: NextPage = () => {
  ...
  const getAllproduct = async () => {
    ...
    const req = await customAxios.get('/api/method/dipmarts_app.api.allproduct', {params: {
      current_page : offset
    }})
        ...
        <div>
          <div className="w-full h-auto md:full md:h-auto grid grid-cols-4 gap-4 md:grid-cols-5 my-[20px]">
            {brandname?.slice(0, 7).map((s) => (
              ...
              <div
                className="flex flex-col bg-[#3080DC] rounded-lg w-full p-auto justify-center"
              >
                <Link href={'/'} passHref>
